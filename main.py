...
intents.members = True  # 欲使用 on_member_join 必須開啟

# ================= 歡迎訊息設定 (持久化) =================
WELCOME_SETTINGS_FILE = os.path.join(os.path.dirname(__file__), 'welcome_settings.json')

def _load_welcome_settings() -> dict:
    """從 JSON 檔案讀取歡迎訊息設定，若不存在則回傳空字典。"""
...
def _save_welcome_settings(data: dict):
    """將歡迎訊息設定安全寫入 JSON 檔案（先寫暫存再替換，避免寫到一半損壞）。"""
    dir_path = os.path.dirname(WELCOME_SETTINGS_FILE)
    try:
        # 使用暫存檔寫入，完成後再 replace，確保原子性
        with tempfile.NamedTemporaryFile(
            'w', encoding='utf-8', dir=dir_path,
...
# 啟動時載入一次，key 為 str(guild_id)
_welcome_settings: dict = _load_welcome_settings()
...
@bot.tree.command(name="active_welcome", description="設定伺服器歡迎訊息的規則與頻道")
...
async def active_welcome(
    ...
    try:
        _save_welcome_settings(_welcome_settings)
    except Exception as e:
        await bot.notify_owner_error(e, interaction, extra_info="active_welcome: 儲存設定失敗")
        await interaction.response.send_message(
            "❌ 儲存設定時發生錯誤，設定可能在重新啟動後遺失，已回報開發者。",
            ephemeral=True
        )
        return
    ...
    desc_lines = [
        f"✅ 歡迎訊息已啟用！",
        f"📢 歡迎頻道：{welcome_channel.mention}",
    ]
    if rules_channel:
...
@bot.tree.command(name="inactive_welcome", description="取消伺服器的歡迎訊息功能")
@app_commands.guild_only()
@app_commands.default_permissions(manage_guild=True)
async def inactive_welcome(interaction: discord.Interaction):
    guild_id = str(interaction.guild.id)
    if guild_id in _welcome_settings:
        del _welcome_settings[guild_id]
        try:
            _save_welcome_settings(_welcome_settings)
        except Exception as e:
            await bot.notify_owner_error(e, interaction, extra_info="inactive_welcome: 儲存設定失敗")
            await interaction.response.send_message(
                "❌ 儲存設定時發生錯誤，可能需要再執行一次，已回報開發者。",
                ephemeral=True
            )
            return
        embed = discord.Embed(
            title="🔕 歡迎訊息已關閉",
            description="此伺服器的歡迎訊息功能已停用。",
            color=discord.Color.red()
        )
    else:
        embed = discord.Embed(
            title="ℹ️ 歡迎訊息未啟用",
            description="此伺服器目前沒有啟用歡迎訊息功能。",
...
async def on_member_join(member: discord.Member):
    ...
    settings = _welcome_settings.get(guild_id)
    if not settings:
        return
    ...
    try:
        ...
        await bot.notify_owner_error(
            e,
            extra_info=f"on_member_join: member={member} guild={member.guild.name} ({member.guild.id})"
        )
